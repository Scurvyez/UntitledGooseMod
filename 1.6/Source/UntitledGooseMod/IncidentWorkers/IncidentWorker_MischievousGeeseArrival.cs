...
namespace UntitledGooseMod.IncidentWorkers
{
    public class IncidentWorker_MischievousGeeseArrival : IncidentWorker
    {
        protected override bool CanFireNowSub(IncidentParms parms)
        {
            ...
            if (!UGMSettings.AllowMischievousGeese || map.dangerWatcher?.DangerRating == StoryDanger.High)
                return false;
            ...
            if (map.mapPawns.AllPawnsSpawned.Count(p => p.def == UGMDefOf.Goose.race) >= 2)
                return false;
            
            return map.mapTemperature.SeasonAndOutdoorTemperatureAcceptableFor(UGMDefOf.Goose.race) 
                   && TryFindBestEntryCell(map, out IntVec3 _);
        }
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            ModExtension_GooseUnhindered ext = gooseKind.race.GetModExtension<ModExtension_GooseUnhindered>();
            
            if (ext == null)
                return false;
            
            int numGeese = Rand.RangeInclusive(ext.incidentNumGeeseMin, ext.incidentNumGeeseMax);
            int exitAfterTicks = Rand.RangeInclusive(ext.incidentExitMapMinTick, ext.incidentExitMapMaxTick);
            MapComponent_MischievousGooseArrival mapComp = map.GetComponent<MapComponent_MischievousGooseArrival>();
